                ...
                @if ($data->count() > 0)
                    @if (MyHelper::tab_class_active_true('trash'))
                        @component('components.dt-bulk-action-all', [
                            'delete_url'  => action('PermissionController@destroyAll', ['tab' => 'trash']),
                            'delete_msg'  => __("$lang.messages.delete_force.all.alert"),
                            'restore_url' => action('PermissionController@restoreAll'),
                            'restore_msg' => __("$lang.messages.restore.all.alert"),
                            'table_name'  => $table_name,
                            ])
                                @include('pages.permission._index.dt-bulk-action-all')
                        @endcomponent
                    @else
                        @component('components.dt-bulk-action-all', [
                            'delete_url' => action('PermissionController@destroyAll'),
    ...
    <div class="row" id="row_show_pagination">
        <div class="col-xl-12">
            @component('components.dt-nav-top', ['data' => $data])
                @slot('bulk_action')
                    @include('pages.permission._index.dt-bulk-action-selected')
                @endslot
            @endcomponent
        </div>
    </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-hover table-sm">
                    @component('components.dt-thead', ['child' => "item-$table_name"])
                        @include('pages.permission._index.dt-thead')
                    @endcomponent
                    <tbody>
                        @forelse ($data as $row => $item)
                            <tr class="tr-show" data-href="{{ action('PermissionController@show', encrypt($item->id)) }}" data-modal="false" data-target="#{{ $table_name }}_modal">
                                ...
                                @include('pages.permission._index.dt-tbody')
                                @if (MyHelper::tab_class_active_true('trash'))
                                    ...
                                    <td class="text-center nowrap td-disable">
                                        @component('components.dt-column-action-adv', [
                                            'open_url'    => action('PermissionController@show', encrypt($item->id)),
                                            'edit_url'    => action('PermissionController@edit', encrypt($item->id)),
                                            'delete_url'  => action('PermissionController@destroy', encrypt($item->id)),
                                            'delete_msg'  => __("$lang.messages.delete_force.alert", ['attr' => $item->name]),
                                            'restore_url' => action('PermissionController@restore', encrypt($item->id)),
                                            'restore_msg' => __("$lang.messages.restore.alert", ['attr' => $item->name]),
                                            'row_name'    => "$table_name-" . $loop->iteration,
                                        ])
                                            @include('pages.permission._index.dt-item-action')
                                        @endcomponent
                                    </td>
                                @else
